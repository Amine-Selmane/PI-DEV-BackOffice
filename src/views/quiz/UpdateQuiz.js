  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/quiz/getQuizById/${id}`)
      .then(response => {
        ...
        const { course, questions, date, beginTime, endTime } = quizData;
...
//   const { id } = useParams();
//   const [formData, setFormData] = useState({
//     studentGrade: '', // Ajouter studentGrade dans l'état initial
//     courseName: '',
//     selectedQuestions: [],
//     allQuestions: [],
//     date: '',
...
//         const formattedDate = new Date(date).toISOString().split('T')[0];
//         const questionContents = questions.map(question => ({ _id: question._id, content: question.content }));
//         const selectedQuestionIds = questions.map(question => question._id);
//         setFormData({
//           studentGrade: student.grade, // Définir studentGrade après récupération des données du quiz
//           courseName: course.name,
...
//   const handleSubmit = e => {
//     e.preventDefault();
//     const { studentGrade, date, beginTime, endTime, selectedQuestions } = formData;
//     if (selectedQuestions.length === 0) {
//       console.error('No questions selected');
//       return;
//     }
//     axios.put(`http://localhost:5000/quiz/updateQuiz/${id}`, { studentGrade, date, beginTime, endTime, questionIds: selectedQuestions })
//       .then(response => {
//         console.log('Quiz updated successfully:', response.data.quiz);
//         window.location.href = '/quiz';
//       })
...
//                 >
//                   {formData.allQuestions.map(question => (
//                     <option key={question._id} value={question._id} disabled>{question.content}</option>
//                   ))}
//                 </Input>
//               </FormGroup>
//               <FormGroup>
